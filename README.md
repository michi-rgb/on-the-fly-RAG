...
- **チャンク長さ調整**: GUI上でチャンク長さ（トークン数）をカスタマイズ可能（デフォルト: 100トークン）
- **リアルタイム検索**: Ollama REST APIを使用した高速な埋め込みと検索
- **並列埋め込み処理**: 最大4スレッドでチャンクを並列埋め込み（高速化）
- **自動リトライ**: Ollama接続エラー時に最大5回自動リトライ（指数バックオフ）
...
`run.bat` をダブルクリックするか、ターミナルで実行するだけです。

```bat
run.bat
```

`run.bat` は以下の処理を順に行います：

1. **Python インストール確認** — Python が PATH に存在しない場合はエラーで終了
2. **Ollama 接続確認** — `localhost:11434` に接続できない場合は警告を表示して一時停止
3. **FastAPI サーバー起動** — バックグラウンドで uvicorn を起動
4. **ブラウザ起動** — 3秒待機後、デフォルトブラウザで `http://localhost:8000` を自動オープン
